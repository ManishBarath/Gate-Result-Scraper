...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Automate GATE portal login and fetch candidate results"
    )
    parser.add_argument(
        "--base-url",
        default="https://goaps.iitg.ac.in/login",
        help="GATE portal login URL",
    )
    parser.add_argument(
        "--credentials-csv",
        default="data/credentials.csv",
        help="Path to CSV with headers: enrollment_id,password",
    )
    parser.add_argument(
        "--output-csv",
        default="output/results.csv",
        help="Path where extracted results are appended",
    )
    parser.add_argument(
        "--captcha-mode",
        default="hybrid",
        choices=["manual", "ocr", "hybrid"],
        help="Captcha solving mode",
    )
    parser.add_argument(
        "--headful",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Retry attempts for captcha/login errors",
    )
    return parser
...
def main() -> None:
    ...
    portal_client = PlaywrightPortalClient(
