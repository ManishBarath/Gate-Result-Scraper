...
class PlaywrightPortalClient(PortalClient):
    ...
    def fetch_candidate_result(self, credential: CandidateCredential) -> CandidateResult:
        ...
        # Clear cookies before starting new user's workflow to avoid session leaking
        ...
        try:
            for attempt in range(1, self.max_captcha_attempts + 1):
                ...
                if self._is_login_success(page):
                    ...
                    return CandidateResult(
                        ...
                        status="success",
                        message="Login success and result page parsed",
                        extracted=extracted,
                    )

                error_message = self._read_login_error(page)
                # For automation, OCR will frequently fail math validations.
                # Always retry on ANY error until max attempts are exhausted, 
                # because generic error messages might mask a captcha failure.
                if attempt < self.max_captcha_attempts:
            ...
            return CandidateResult(
                enrollment_id=credential.enrollment_id,
                status="failed",
                message="Max captcha attempts reached",
            )
        except Exception as error:
            return CandidateResult(
                enrollment_id=credential.enrollment_id,
                status="failed",
                message=f"Automation error: {error}",
            )
        finally:
            page.close()
    ...
    def _fill_login_fields(self, page, credential: CandidateCredential) -> None:
        ...
        try:
            page.get_by_label(re.compile(r"Enrollment ID|Email", re.I)).fill(credential.enrollment_id)
    ...
    def _extract_result_data(page) -> dict[str, str]:
        ...
        patterns = {
            "name": r"(?:Candidate Name|Name)\s*[:\-]\s*([^\n]+)",
            "registration": r"(?:Enrollment ID|Registration No\.?|Application No\.?)\s*[:\-]\s*([^\n]+)",
            "marks": r"(?:Marks|Score)\s*[:\-]\s*([^\n]+)",
            "rank": r"(?:AIR|Rank)\s*[:\-]\s*([^\n]+)",
            "gate_score": r"(?:GATE Score)\s*[:\-]\s*([^\n]+)",
        }
