...
class ConsoleResultSink(ResultSink):
    ...
    def publish(self, result: CandidateResult) -> None:
        print(f'[{result.status.upper()}] {result.enrollment_id}: {result.message}')
        if result.extracted:
...
class CsvResultSink(ResultSink):
    ...
    def publish(self, result: CandidateResult) -> None:
        ...
        with self._output_path.open('a', encoding='utf-8', newline='') as csv_file:
            writer = csv.DictWriter(csv_file, fieldnames=['fetched_at', 'enrollment_id', 'status', 'message', 'extracted'])
            if not file_exists:
                writer.writeheader()
            writer.writerow({'fetched_at': result.fetched_at, 'enrollment_id': result.enrollment_id, 'status': result.status, 'message': result.message, 'extracted': result.extracted})
