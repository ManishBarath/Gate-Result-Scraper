...
class MathExpressionParser:
    ...
    def parse(self, raw_text: str) -> str:
        ...
        # STRICT VALIDATION: If it doesn't strictly match "Number Operator Number", reject immediately.
        # This prevents submitting nonsense like "84", forcing an automatic instant refresh instead.
    ...
    def _normalize(raw_text: str) -> str:
        ...
        text = text.replace("÷", "/")
        text = text.replace("t", "+")  # ddddocr sometimes misreads '+' as 't'
...
class OcrMathCaptchaSolver(CaptchaSolver):
    ...
    def solve(self, image_bytes: bytes) -> str:
        try:
            ...
            raise RuntimeError(
                "OCR dependencies missing. Install ddddocr and opencv-python-headless."
            ) from error

        # Initialize ddddocr in the solver
        ...
        # Basic grayscale and thresholding to make the image clearer
        grayscale = cv2.cvtColor(source, cv2.COLOR_BGR2GRAY)
        _, thresh = cv2.threshold(grayscale, 120, 255, cv2.THRESH_BINARY)
        
        # 1. ADVANCED PREPROCESSING: The distractor line is thin, numbers are thick.
        # We use an "erosion" or "opening" operation. This erodes away thin white pixels
        # (the diagonal line) but preserves the thicker white numbers.
        kernel = np.ones((2, 2), np.uint8)
        clean_image = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel)

        # 2. INVERT THE IMAGE: Most OCR models (including ddddocr/Tesseract) are trained 
        # on black text against a white background. The original is white on black!
        final_image = cv2.bitwise_not(clean_image)

        # DEBUG: Save the processed image to disk so you can see exactly what the OCR sees!
        cv2.imwrite("output/debug_captcha.png", final_image)
        
        # Re-encode the preprocessed image for the OCR engine
