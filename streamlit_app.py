...
def load_and_clean_data(file_path="data/Data.csv"):
    ...
    # Handle standardizing the 'Username' and 'Password' columns specifically
    if 'Username' in df.columns:
        df['Username'] = df['Username'].astype(str).str.replace(r'\s+', '', regex=True) # remove newlines, spaces
    
    if 'Password' in df.columns:
        # Passwords might have strict chars, but should not have leading/trailing newlines or spaces
...
def worker_verify_credential(row_tuple):
    ...
    # Each thread MUST instantiate its own Playwright client to be thread-safe
    solver = CaptchaSolverFactory.create("ocr")
    client = PlaywrightPortalClient(
        base_url="https://goaps.iitg.ac.in/login",
...
with tabs[0]:
    ...
    # User controls for number of retries internally for Captcha
    ...
    if start_check:
        ...
        # Area to showcase wrong users incrementally
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            # iterrows returns (index, Series) which we pack into a tuple
